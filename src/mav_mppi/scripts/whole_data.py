...
class controller:
    def __init__(self):
        ...
        self.logger = DataLogger("/home/chan/aerial_ws/src/mav_mppi/scripts/data/time_horizon")
    ...
    def save_data(self, oMi):
        eef_pos = oMi.translation.copy()
        eef_vel = self.v[3:6].copy()   
        eef_rpy = R.from_matrix(oMi.rotation).as_euler('zyx', degrees=False)

        target_pose = self.mppi.target_pose.clone()
        xyz = target_pose._Pose__pose.cpu().numpy().copy()           # (3,)
        quat = target_pose._Pose__orientation.cpu().numpy().copy()    # (4,)
        target_xyzquat = np.concatenate([xyz, quat])          # (7,)
        target_euler = xyzquat_to_xyzrpy(target_xyzquat)

        target_pos = target_euler[:3].copy() 
        taget_rpy = target_euler[3:].copy() 
        drone_pos = self.q[:3].copy()
        ...
        self.logger.append({
            ...
            "target_rpy" : taget_rpy,
            "drone_pos" : drone_pos
        })

        self.logger.save("test")
