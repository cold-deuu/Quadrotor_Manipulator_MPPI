...
class controller:
    def __init__(self):
        ...
        rospy.on_shutdown(self.on_shutdown)  # 종료 시 저장 등록
    ...
    def main(self):
        while not rospy.is_shutdown():
            ...
            if self.q is not None:
                ...
                self.pos_log.append(trans)
                self.vel_log.append(vel)
                self.xdes_log.append(xdes[:3].detach().cpu().numpy() if hasattr(xdes, 'detach') else xdes[:3])
                self.rpy_log.append(self.q_euler[3:6].copy())
    ...
    def on_shutdown(self):
        base_path = "/home/chan/aerial_ws/src/mav_mppi/scripts/data/"
        log_file = os.path.join(base_path, "drone_log.csv")
