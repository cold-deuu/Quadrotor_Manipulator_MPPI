...
public: void computeQuadControl(DerivedPose foundpose, double desiredState[8], double yaw_mom, Eigen::VectorXd* rotor_velocities)
	{  
		...
		*/
		...
		//  GAINS //
		// double Kpx=3;   double Kpy=3;   double Kpz=5.0;  

		// double Kdx=5.0;   double Kdy=5.0;   double Kdz=3; 
		...
		double yerr = ydes - y;		
		double zerr = zdes - z;
		double x_int = integral(xerr,prev_Perr(0));
		double y_int = integral(yerr,prev_Perr(1));
		double z_int = integral(zerr,prev_Perr(2));


		double U1 = mz_hat * (9.81 + Kpz * zerr - Kdz * z_d + Kiz * z_int)/(cos(phi)*cos(theta));
		double ux = mx_hat/U1 * (Kpx * xerr - Kdx * x_d + Kix * x_int);
		double uy = my_hat/U1 * (Kpy * yerr - Kdy * y_d + Kiy * y_int);
		...
		double phierr = phides - phi;
		double thetaerr = thetades - theta;
		double psierr = psides - psi;
		
		double U2 = (Ixx/xlen)*(Kpph * phierr + Kdph * (phiddes - phi_d)) + (1/xlen)*((Izz-Iyy)*theta_d*psi_d)  ;
		double U3 = (Iyy/ylen)*(Kpth * thetaerr + Kdth * (thetaddes - theta_d)) + (1/ylen) * ((Ixx - Izz)*phi_d * psi_d);
		double U4 = (Izz) * (Kpps * psierr - Kdps * psi_d) + (Iyy - Ixx) *phi_d *theta_d;
		...
		prev_Perr(0) = xerr;	prev_Perr(1) = yerr;	prev_Perr(2) = zerr;
