...
def xyz_to_se3(xyz : torch.Tensor) -> torch.Tensor:
    # Assumption : rpy = zeros(3)    
    n_samples, n_timestep = xyz.shape[:2]
    device = xyz.device

    se3 = torch.eye(4, device=device).unsqueeze(0).unsqueeze(0)  # (1, 1, 4, 4)
    se3 = se3.expand(n_samples, n_timestep, 4, 4).clone()        # (n_samples, n_timestep, 4, 4)
