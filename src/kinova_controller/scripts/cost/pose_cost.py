...
class PoseCost():
    ...
    def compute_stage_cost(self, eefTraj, target_pose) -> torch.Tensor:
        ...
        diff_pose = ee_sample_pose - target_pose.pose.to(device=self.device) # diff pose : N,T,3


        diff_pose = ee_sample_pose - target_pose.pose.to(device=self.device)
        diff_pose_square = diff_pose **2
        cost_pose = torch.sum(diff_pose_square, dim=-1)
        ...
        diff_orientation_abs = torch.abs(diff_orientation)
        cost_orientation = torch.sum(diff_orientation_abs, dim=-1)


        # print(f"Pose Diff : {diff_pose}")
    ...
    def compute_drone_pose_cost(self, qSamples, eefTraj):
        ...
        constraint_idx = drone_z - 0.1 < eef_z
